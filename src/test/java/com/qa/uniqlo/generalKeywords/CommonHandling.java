package com.qa.uniqlo.generalKeywords;
...
//            Log.info("VERIFY IF {"+ expTxt+ "} CONTAINS {"+ toBeVerifiedTxt+ "}");
    ...
    public static void verifyIfStringIsEqual(@NotNull String actTxt, @NotNull String expTxt) {
        String expTxtNormalized= expTxt.trim().toLowerCase();
        String actTxtNormalized= actTxt.trim().toLowerCase();
        Log.info("VERIFY IF {"+ actTxt+ "} IS EQUAL TO {"+ expTxt+ "}");
        Assert.assertEquals(actTxtNormalized, expTxtNormalized);
    }
...
//        AbstractTest.page.locator("").hover();
    ...
    public static void waitForPageToLoad(@NotNull LoadState loadState, @Nullable int timeOut) throws Exception{
...
//            AbstractTest.page.waitForLoadState(Constants.LOAD_STATE, Constants.MAXTIMEOUT);
    ...
    @Contract(pure = true)
    public static @Nullable String getTitle() throws Exception {
        waitForPageToLoad(Constants.DOM_CONTENT_LOADED_STATE, 2);
        String titleTxt= AbstractTest.page.title();
        return titleTxt;
    }

    public static @Nullable String getUrl() throws Exception {
        waitForPageToLoad(Constants.LOAD_STATE, 2);
        String urlTxt= AbstractTest.page.url();
        return urlTxt;
    }
